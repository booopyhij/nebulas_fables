...
function wikiSearch(searchInput){
    ...
    var url = 'https://en.wikipedia.org/w/api.php?action=query&prop=categories&list=search&titles='+ searchInput + '&format=json&srsearch=' +searchInput + '&origin=*' ;

fetch(url)
    .then(function(response){return response.json();})
    //.then(function(response) {console.log(response);})
    //.then(function(data){return data.innerText;})
    .then(function(data) {console.log(data.query.search[0].snippet);
        console.log(data);
        $('#articleSearch').empty();
